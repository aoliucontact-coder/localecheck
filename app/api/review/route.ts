...
import {buildReviewMessages,parseReviewResponse,validateReviewRequest} from '@/lib/ai-review.mjs';
export async function POST(request:Request){
 ...
 const raw=await request.text();if(new TextEncoder().encode(raw).byteLength>1_000_000)return Response.json({error:'请求过大。'},{status:413});
 let data;try{data=validateReviewRequest(JSON.parse(raw))}catch(error){return Response.json({error:(error as Error).message},{status:400})}
 ...
 const response=await fetch(upstream,{method:'POST',signal:AbortSignal.timeout(45000),headers:{Authorization:`Bearer ${config.LLM_API_KEY}`,'Content-Type':'application/json'},body:JSON.stringify({model:config.LLM_MODEL,messages:buildReviewMessages(data)})});
 ...
 return Response.json({issues:parseReviewResponse(content,ids,{model:config.LLM_MODEL})});
