...
const dataset={name:'rules-dev',provenance:'synthetic',split:'development',description:'合成开发集',glossary:[{source:'工作区',target:'workspace'}],cases:[{id:'1',source:'保存',target:'Save',context:'按钮',expectedCategories:[]}]};

test('valid rule dataset is accepted',()=>assert.equal(validateRuleDataset(dataset),dataset));
test('duplicate case ids and labels are rejected',()=>{
 assert.throws(()=>validateRuleDataset({...dataset,cases:[dataset.cases[0],dataset.cases[0]]}));
 assert.throws(()=>validateRuleDataset({...dataset,cases:[{...dataset.cases[0],expectedCategories:['数字','数字']}]}));
});
test('unknown labels and duplicate glossary sources are rejected',()=>{
 assert.throws(()=>validateRuleDataset({...dataset,cases:[{...dataset.cases[0],expectedCategories:['准确性']}]}));
 assert.throws(()=>validateRuleDataset({...dataset,glossary:[dataset.glossary[0],dataset.glossary[0]]}));
});
