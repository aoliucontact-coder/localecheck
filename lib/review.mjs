...
export function parseCSV(input){
 if(typeof input!=='string'||input.length>1_000_000)throw Error('文件超过 1 MB 限制。');
...
export function reviewRows(rows,glossary=[]){const issues=[];for(const r of rows){const add=(category,reason)=>issues.push({id:r.id,category,reason,origin:'规则',suggestion:''});if(!r.target.trim()){add('漏译','英文译文为空，请补充译文。');continue}if(!same(placeholders(r.source),placeholders(r.target)))add('占位符',`原文：${placeholders(r.source).join('、')||'无'}；译文：${placeholders(r.target).join('、')||'无'}。名称和数量需一致。`);if(!same(numbers(r.source),numbers(r.target)))add('数字',`原文：${numbers(r.source).join('、')||'无'}；译文：${numbers(r.target).join('、')||'无'}。请核对数值与单位。`);for(const g of glossary){const escaped=g.target.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');if(r.source.includes(g.source)&&!new RegExp('(?:^|[^A-Za-z0-9_])'+escaped+'(?=$|[^A-Za-z0-9_])','i').test(r.target))add('术语',`“${g.source}”指定译法为“${g.target}”。`)}}return issues}
